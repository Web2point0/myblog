...
async function fetchHiscores(username) {
  let proxyURL;
  if (GAME_MODE === "rs3") {
    proxyURL = `https://api.allorigins.win/raw?url=https://secure.runescape.com/m=hiscore/index_lite.ws?player=${encodeURIComponent(username)}`;
  } else {
    proxyURL = `https://api.allorigins.win/raw?url=https://secure.runescape.com/m=hiscore_oldschool/index_lite.ws?player=${encodeURIComponent(username)}`;
  }
